...
function _make_chart(){
  ...
  $vars['file'] = base64_encode(serialize($cache_file));              
	$theme_folder_url = $this->config->item('theme_folder_url');
	if (substr($theme_folder_url, -1) != '/') $theme_folder_url .= '/';
  $vars['theme_path'] = $theme_folder_url.'third_party/jb_graphite/js/';
	
	return $this->load->view('../third_party/jb_graphite/chart',$vars,TRUE);                                                                                                                      
